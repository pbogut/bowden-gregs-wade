...
module WadeL_double_helix(){
	...
	difference(){
		...
		translate([0,0,3])rotate([180,0,0])m5_hole_vert_with_hex(100);
	}
}
module WadesL(){
	//Large WADE's Gear
...
module m5_hole_vert_with_hex(l) {
	m5_clearance_rad=2.6;

	// copied from mendel misc
	module box(w,h,d) {
	...
	module m5_nut_cavity(l) {
		hexagon(height=8,depth=l);
	}

	module m5_hole_vert(l) {
		cylinder(l,m5_clearance_rad,m5_clearance_rad,center=true);
	}
	union () {
		m5_hole_vert(l);
		translate ([0,0,-l/4]) rotate ([0,0,30]) m5_nut_cavity(l/2);
	}
}
